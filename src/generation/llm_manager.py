		...
		if active_service in ("google", "gemini"):
				...
				try:
					...
					if response and response.content:
						...
						logging.getLogger(__name__).warning(
							f"[LLMManager] Google phản hồi nội dung rỗng. Đang chờ 5s để gửi lại câu hỏi (Lần thử {attempt})..."
						)
						time.sleep(5)
					...
					logging.getLogger(__name__).warning(
						f"[LLMManager] {msg} (Lần thử {attempt} - Giữ tiến trình để xử lý xong câu hỏi)"
					)
			...
			while True:
				...
				try:
					...
					logging.getLogger(__name__).warning(
						f"[LLMManager/NVIDIA] Máy chủ gặp sự cố ({err_str[:90]}). Đang chờ {int(wait_time)}s để thử lại lần {attempt} (Không bỏ qua câu hỏi)..."
					)
					time.sleep(wait_time)
			...
			while True:
				...
				try:
					...
					logging.getLogger(__name__).warning(
						f"[LLMManager/Groq] Máy chủ gặp sự cố ({err_str[:90]}). Đang chờ {int(wait_time)}s để thử lại lần {attempt} (Không bỏ qua câu hỏi)..."
					)
					time.sleep(wait_time)
