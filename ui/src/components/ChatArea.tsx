...
const SAMPLE_PROMPTS = [
  'Hạn mức giao đất nông nghiệp và đất ở theo Luật Đất đai 2024?',
    ...
    return (
      <div className="flex-1 overflow-y-auto p-4 sm:p-8 flex flex-col items-center justify-center text-center">
        ...
        <h2 className="text-xl font-bold text-gray-100 mb-2">
          Trợ lý Pháp luật Đất đai 2024
        </h2>
        <p className="text-gray-400 text-sm max-w-md mb-8">
          Hệ thống hỏi đáp chuyên sâu <strong>Luật Đất đai 2024</strong> với pipeline RAG linh hoạt (Vector DB BGE-M3, Contriever, Graph Database, RAG-Fusion, Filter-then-Rerank).
        </p>
    ...
    <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
      ...
      {isLoading && (
        <div className="flex gap-3.5 max-w-4xl mx-auto items-start">
          <div className="w-8 h-8 rounded-xl border border-accentGreen/30 bg-accentGreen/20 text-accentGreen flex items-center justify-center flex-shrink-0 mt-1">
          ...
          <div className="bg-cardBg border border-borderDark/60 rounded-2xl rounded-tl-sm px-4 py-3 text-xs text-gray-300 flex items-center gap-2.5 shadow-sm">
            <span className="inline-block w-2 h-2 rounded-full animate-ping flex-shrink-0 bg-accentGreen" />
            <span>Đang tra cứu cơ sở dữ liệu luật và tổng hợp câu trả lời...</span>
          </div>
        </div>
      )}
